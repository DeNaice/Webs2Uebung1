...
async function handleQuery(query: string, data: any[]): Promise<any> {
...
app.post('/logout', (req: Request, res: Response) => {
  // Log out user
  req.session.user = null; // Delete user from session
  res.status(200).send({
    message: 'Successfully logged out',
  });
});
...
app.delete('/user/:userId', isLoggedIn(), async (req: Request, res: Response) => {
  ...
  try {
    const result = await handleQuery(query, [userId]);
    ...
    res.status(500).send({
      message: 'Database request failed: ' + e
    });
  }
});
